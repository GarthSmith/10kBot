...
namespace Ubiety.Infrastructure
{
    ...
    internal static class ProtocolParser
    {
        ...
        {
            LogQueue.Log("Attempting to parse " + message);
            bool fullStream = false;

            if (ProtocolState.State is DisconnectedState)
            {
                LogQueue.Warn("Skipping parsing because we are disconnected.");
                return;
            }
            ...
            {
                LogQueue.Log("Received end of stream xml! Closing stocket.");
            ...
            {
                LogQueue.Error("Error in xml from server. Received " + e);
            ...
            {
                LogQueue.Error("Invalid operation parsing incoming message. Received " + e);
        ...
        {
            if (_element != null) _element.AppendChild(Tag.Document.CreateTextNode(_reader.Value));
        }
