...
namespace Ubiety.Registries
{
    ...
    public static class TagRegistry
    {
        ...
        {
            LogQueue.Log("Loading tags from assembly " + assembly.FullName);

            IEnumerable<XmppTagAttribute> tags = assembly.GetAttributes<XmppTagAttribute>();
            foreach (XmppTagAttribute tag in tags)
            {
                LogQueue.Log("Loading tag " + tag.Name + " as class " + tag.ClassType.FullName + " in the " + tag.Namespace + " namespace.");
        ...
        public static T GetTag<T>(XmlQualifiedName qname) where T : Tag
        {
            ...
            if (RegisteredItems.TryGetValue(qname.ToString(), out t))
            {
                ...
                if (ctor == null)
                {
                    LogQueue.Log("Found tag " + qname + " which corresponds to type " + t + ". Using XmlQualifiedName.");
                    ctor = t.GetConstructor(new[] {typeof (XmlQualifiedName)});
                    if (ctor != null) tag = (T) ctor.Invoke(new object[] {qname});
                }
                else
                {
                    LogQueue.Log("Found tag " + qname + " which corresponds to type " + t);
