	...
	{       
...
#if SSL || MONOSSL || BCCRYPTO || UNITY_5
			set
			{
                // Only one of both can be true
				m_UseStartTLS = value;
		...
		*/
		...
		public override void StreamParserOnStreamElement(object sender, Node e)
		{
			...
			else if ( e.GetType() == typeof (agsXMPP.protocol.stream.Features) )
			{
                // Stream Features
                // StartTLS stuff
                agsXMPP.protocol.stream.Features f = e as agsXMPP.protocol.stream.Features;
                if (m_UseCompression &&
